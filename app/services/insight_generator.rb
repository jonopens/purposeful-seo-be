class InsightGenerator
	...
	def text_html_ratio_case_check
		...
		when self.page.text_to_html_ratio == 0
			Insight.create(crawl_id: self.crawl.id, page_id: self.page.id, optimization_type: "text to html ratio", content: "This page should have some text on it. Without text, it's hard for search engines to understand what a page is about.")
		when self.page.text_to_html_ratio < 20
			Insight.create(crawl_id: self.crawl.id, page_id: self.page.id, optimization_type: "text to html ratio", content: "Consider adding more text content to this page. The ratio of text to HTML code is low.")
		end
	end
