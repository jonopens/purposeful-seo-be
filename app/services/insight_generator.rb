class InsightGenerator
	...
	def title_case_check
		...
		when self.page.title.length > 65
			Insight.create(crawl_id: self.crawl.id, optimization_type: "title", content: "Title tag is too long. Title tags longer than 65 characters should be shorter.")
		end
	end
	...
	def text_html_ratio_case_check
		...
		when self.page.text_to_html_ratio == 0
			Insight.create(crawl_id: self.crawl.id, optimization_type: "text to html ratio", content: "This page should have some text on it. Without text, it's hard for search engines to understand what a page is about.")
		when self.page.text_to_html_ratio < 20
			Insight.create(crawl_id: self.crawl.id, optimization_type: "text to html ratio", content: "Consider adding more text content to this page. The ratio of text to HTML code is low.")
		end
	end
